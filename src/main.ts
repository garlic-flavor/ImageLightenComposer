window.onload = ():void=> {
    document.getElementById("button_lightest")!.onclick = ():void=>{
        document.getElementById("result")!.innerHTML = '';
        document.getElementById("storage")!.innerHTML = '';
        let progress = document.getElementById("progress")!;
        progress.innerHTML = '';

        let btn_files = document.getElementById("image_files") as HTMLInputElement;
        if (btn_files === null || btn_files.files === null || btn_files.files.length == 0) { return; }
        var jobs = [];
        ...
        Promise.all(jobs).then(():void=>{
            processLightest().then(():void=>{
                console.log("done");
            });
        });
    };
    document.getElementById("button_save")!.onclick = ():void => {
...
function loadToCanvas(file: File): Promise<void> {
...
async function processLightest(): Promise<void> {
    let div = document.getElementById("storage")!;
    ...
    for (var i=0; i < images.length; i++) {
        ...
        for (var y = 0; y < height; y++) {
            for (var x = 0; x < width; x++) {
                ...
                let srcR = src.data[idx*4] as number;
                let srcG = src.data[idx*4+1] as number;
                let srcB = src.data[idx*4+2] as number;
        ...
        if (p!==null && 0 < p.length) {
            let span = p[0] as HTMLSpanElement;
            span.innerHTML = '●';
